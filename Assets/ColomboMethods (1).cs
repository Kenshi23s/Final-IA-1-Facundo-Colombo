...
namespace FacundoColomboMethods
{
    ...
    public static class ColomboMethods
    {
        ...
        {
            // Crea un array de componentes del tipo T con una longitud igual al número de hijos del transform padre

            T[] Components = new T[Father.childCount];
            // Repite el proceso para cada uno de los hijos del transform padre
            for (int i = 0; i < Father.childCount; i++)
            {
                // Obtiene el i-ésimo hijo del transform padre y Obtiene el componente del tipo T conectado al hijo
                var item = Father.transform.GetChild(i).GetComponent<T>();

                // Si se encontró el componente, agrégalo al array de componentes
                if (item != null)
                {
                    Components[i] = item;
                }
        ...
        {
            // Índice del componente más cercano
            int nearestIndex = 0;

            // Magnitud de la distancia del componente en la posición 0 al punto dado
            ...
            {
                // Magnitud de la distancia temporal desde el componente en la posición i hasta el punto dado
                float tempMagnitude = (objPosition[i].transform.position - myPos).magnitude;

                // Si la distancia temporal es más corta que la distancia más corta encontrada hasta el momento, actualiza los valores
            ...
            // Devuelve el componente más cercano
